...
@Component
public class CompatDownloadFileRepository {
    @Value("${fileservice.collect.ftp.downloadDirectory}")
    String downloadDirectory;

    ObjectMapper objectMapper = new ObjectMapper();

    Map<String, VFtpCompatDownloadRequest> requestMap = new HashMap<>();

    String getRequestDirectory(String requestNo){
        return requestNo;
    }

    String getRequestDirectory(VFtpCompatDownloadRequest request){
        return getRequestDirectory(request.getRequestNo());
    }

    public void addRequest(VFtpCompatDownloadRequest request) throws IOException {
    ...
    synchronized void writeRequest(VFtpCompatDownloadRequest request, File file) throws IOException{
        try {
    ...
    public void writeRequest(VFtpCompatDownloadRequest request) throws IOException{
    ...
    public void endRequestExecution(VFtpCompatDownloadRequest request){
        synchronized (requestMap) {
            requestMap.remove(request.getRequestNo());
        }
    }

    public VFtpCompatDownloadRequest readRequest(File target) throws Exception{
        if (target == null){
        ...
        return objectMapper.readValue(target, VFtpCompatDownloadRequest.class);
    }

    public VFtpCompatDownloadRequest getRequest(String requestNo){
        VFtpCompatDownloadRequest request = requestMap.get(requestNo);
        if (request == null){
    ...
    public void removeRequest(VFtpCompatDownloadRequest request){
        File requestDownDirectory = new File(downloadDirectory, getRequestDirectory(request));
        File requestFile = new File(requestDownDirectory, request.getRequestNo() + ".json");
