...
echo "##########################################################"
echo "## Install Rapid-Collector(ESP) $RAPIDCOLLECTOR_VERSION ##"
echo "##########################################################"
...
echo "##########################################################"
echo "  Disable 2020 Phase3 Tomcat9, Postgres."
echo "##########################################################"
...
then
    ...
    echo "Disable 2020 Phase3 Postgres"
    ...
    echo "Delete 2020 Phase3 Log Files"
    rm -rf /LOG/downloads
    rm -rf /LOG/wdownloads
    rm -rf /LOG/rssportal
    rm -rf /LOG/autocollect
    rm -rf /LOG/cache
    rm -rf /LOG/zip

    echo "Delete 2020 Phase3 Http Configuration"
    rm -rf /etc/httpd/conf.d/timeout.conf
    rm -rf /etc/httpd/conf.d/rss-accctrl.conf
    rm -rf /etc/httpd/conf.d/fsm.conf
...
if [ ${RET} -ne 0 ]
then
    cp ${CUR}/data/structure.json /CANON/ENV
fi
...
if [ ${RET} -ne 0 ]
then
    cp ${CUR}/data/categories.json /CANON/ENV
fi
...
if [ ${RET} -ne 0 ]
then
    cp ${CUR}/data/admin_password.json /CANON/ENV
fi
